...
template <class T> uint32_t Shwfs::_cog(T *img, int xpos, int ypos, int w, int h, int stride, uint32_t samini, fcoord_t& cog) {
	uint32_t fi, csum = 0;
	cog.x = cog.y = 0.0;
...
Shwfs::Shwfs(Io &io, string conffile): Wfs(io, conffile) {
	...
	string camcfg = cfg.getstring("camcfg");
	if (camcfg[0] != '/') camcfg = conffile.substr(0, idx) + "/" + camcfg;
	cam = Camera::create(io, camcfg);
	
	name = cfg.getstring("name");
	subap.x = cfg.getint("subapx");
	subap.y = cfg.getint("subapy");
	
	sasize.x = cam->get_width() / subap.x;
	sasize.y = cam->get_height() / subap.x;
	
	track.x = sasize.x * (int) cfg.getdouble("trackx", 0.5);
	track.y = sasize.y * (int) cfg.getdouble("tracky", 0.5);
	
	samaxr = cfg.getint("samaxr", -1);
	samini = cfg.getint("samini", 30);
	...
	shifts = new coord_t[subap.x * subap.y];
