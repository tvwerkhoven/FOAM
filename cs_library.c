...
void corrTrack(int wfs, float *aver, float *max, float coords[][2]) {
	// this will contain correlation tracking, but first we need to get reference images
	  ...
	  // int ci, ndx, ndy, dx=0, dy=0;
#define NP 5
#define NO NP/2
	  int d, diff[NP][NP];
	  float sig[NP];
	  static float refaver = 0.0, corr = 1.0;
	  // uint16_t g;
	  // float sumx, sumy;
	  ...
	  uint16_t *gp, *dp; // pointers to gain and dark
	  uint8_t  *ip, *cp, *rp; // pointers to raw, processed image, reference

	  // --->>> might not really need these alignments, check it!
	  ...
	  *max = 0; // set maximum of all raw subapertures to 0
	  ...
	  for (sn=0;sn<nsubap;sn++) {
	    ...
	    // dark and flat correct subaperture, determine statistical quantities
	    imcal(cp, ip, NULL, NULL, sp, tmpmean4, max8);
	    ...
	    switch (axes) {
	    case AO_AXES_XY:
	      ip = &cimage[sn*SX*SY]; // first pixel of each subaperture
	      for (ix=-NO;ix<=NO;ix++) {
	    ...
	    if ((axes==AO_AXES_X) || (axes==AO_AXES_XY)) {
	      if (axes==AO_AXES_XY) { // average values over the y-axis
		for (ix=0;ix<NP;ix++) { 
		  sig[ix] = 0.0;
	    ...
	    if ((axes==AO_AXES_Y) || (axes==AO_AXES_XY)) {
	      if (axes==AO_AXES_XY) { // average values over the x-axis
		for (iy=0;iy<NP;iy++) {
		  sig[iy] = 0.0;
